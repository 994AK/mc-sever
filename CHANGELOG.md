...
- `/menutool` 改为 CMI 发放菜单钟，并新增 LeafMenuTool 0.1.0 监听命名菜单钟右键打开 `/menu`，不再依赖 CommandGUI tool。
...
- 再次按玩家反馈收敛快捷菜单：主菜单改为玩家便捷、传送点、游戏功能三类；补上资源回收站、连锁采集说明和假人常用入口；菜单 lore 标明插件来源和命令格式；所有 active 菜单项显式配置左键/右键命令；说明类按钮不关闭菜单，外部 GUI 入口不再先关闭当前菜单。
- 生存传送模块新增随机传送入口，开放普通玩家 `/rt` 并启用 `/rtp` 别名；随机传送仅开放主世界范围，避免下界和末地误传送。
- 随机传送主世界范围从 500-1000 格扩大为环形 1500-5000 格。
