...
public class FakeplayerConfig extends PluginConfig {
     ...
     * 服务器列表 ping 人数是否扣除假人
     */
    private boolean hideFromServerListPingCount;
    ...
    private Map<String, PlayerSpawnPolicy> playerPolicies;
    ...
    protected void reload(@NotNull FileConfiguration file) {
        ...
        this.playerPolicies = getPlayerPolicies(file);
        ...
        this.hideFromPlayerList = file.getBoolean("hide-from-player-list", false);
        this.hideFromServerListPingCount = file.getBoolean("hide-from-server-list-ping-count", false);
    ...
    public @NotNull PlayerSpawnPolicy getPlayerSpawnPolicy(@NotNull CommandSender creator) {
        if (!(creator instanceof Player player)) {
            return PlayerSpawnPolicy.DEFAULT;
        }

        var policies = this.playerPolicies == null ? Map.<String, PlayerSpawnPolicy>of() : this.playerPolicies;
        var uuidPolicy = policies.get(player.getUniqueId().toString().toLowerCase(Locale.ENGLISH));
        if (uuidPolicy != null) {
            return uuidPolicy;
        }
        return policies.getOrDefault(player.getName().toLowerCase(Locale.ENGLISH), PlayerSpawnPolicy.DEFAULT);
    }

    public int getPlayerLimit(@NotNull CommandSender creator) {
        return this.getPlayerSpawnPolicy(creator).resolveLimit(this.getPermissionLimit(creator).orElse(this.playerLimit));
    }

    private @NotNull Optional<Integer> getPermissionLimit(@NotNull CommandSender creator) {
        if (!(creator instanceof Player player)) {
        ...
        for (var permission : player.getEffectivePermissions()) {
            ...
            var node = permission.getPermission().toLowerCase(Locale.ENGLISH);
            if (!node.startsWith(Permission.limitPrefix)) {
                continue;
            }

            var rawValue = node.substring(Permission.limitPrefix.length());
            try {
                limit = Math.max(limit, maxIfZero(Integer.parseInt(rawValue)));
            } catch (NumberFormatException ignored) {
                // Ignore malformed dynamic permission nodes such as fakeplayer.limit.vip.
            }
        }
        return limit < 0 ? Optional.empty() : Optional.of(limit);
    }

    private @NotNull Map<String, PlayerSpawnPolicy> getPlayerPolicies(@NotNull FileConfiguration file) {
        var section = file.getConfigurationSection("player-policies");
        if (section == null) {
            return Map.of();
        }

        var policies = new HashMap<String, PlayerSpawnPolicy>();
        for (var key : section.getKeys(false)) {
            var path = "player-policies." + key;
            if (!file.isConfigurationSection(path)) {
                continue;
            }

            var limit = file.isSet(path + ".player-limit")
                    ? maxIfZero(file.getInt(path + ".player-limit"))
                    : null;
            policies.put(
                    key.toLowerCase(Locale.ENGLISH),
                    new PlayerSpawnPolicy(
                            file.getBoolean(path + ".spawn-enabled", true),
                            limit,
                            file.getString(path + ".reason", "")
                    )
            );
        }
