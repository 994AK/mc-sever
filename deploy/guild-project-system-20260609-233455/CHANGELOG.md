...
- 新增 `/projects`、`/daily`、`/guild`、`/proposal`、`/projectrewards`、`/projectsubmit` 游戏内入口，配合 CMI CustomText 说明本周项目、提案、奖励边界和验收提交。
- 快捷菜单中间行改为 `本周项目`、`每日任务`、`公会小队`，替换原飞行开关、飞行能量和飞行速度入口。
- 关闭 CMI flight charge 模块，移除普通玩家 `/flyc`、`/flightcharge`、`/flyspeed` 权限，不再每 30 分钟自动发放飞行充能。
...
- 快捷菜单第三行改为普通 Residence 圈地模块，包含圈地步骤、领取木锄、快速 48x48 选区、列表、限制、授权、删除和规范入口。
