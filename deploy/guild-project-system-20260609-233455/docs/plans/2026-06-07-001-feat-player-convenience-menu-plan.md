...
> Superseded note, 2026-06-09: this plan describes the earlier menu version that exposed flight charge entries. The current guild project daily activity rollout replaces those menu slots with `本周项目`, `每日任务`, and `公会小队`, disables CMI flight charge for default players, and removes `/flyc`, `/flightcharge`, and `/flyspeed` from the default group. Treat `plugins/CommandGUI/config.yml`, `plugins/LuckPerms/yaml-storage/groups/default.yml`, and `docs/plans/2026-06-09-guild-project-daily-activity-rollout-plan.md` as the current source of truth.
